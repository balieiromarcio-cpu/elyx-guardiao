    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {products.map((p) => {
          ...
          return (
            <Link key={p.id} href={`/dashboard/produtos/${p.slug}`} className="card card-hover flex flex-col gap-3 p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <span className={`badge ${p.status === "ATIVO" ? "badge-ok" : p.status === "DESCONTINUADO" ? "badge-muted" : "badge-warn"}`}>{p.status.toLowerCase()}</span>
              </div>
